...
const spotLineDistance = function (beginline: xy, endline: xy, node: xy): string {
  ...
  if (lineDistance(beginline, node) <= 8) {
    ...
    if (
      node.x <= Math.max(beginline.x, endline.x) + 8 &&
      node.x >= Math.min(beginline.x, endline.x - 8)
    ) {
      //判断x坐标是否在线段内
      if (
        node.y <= Math.max(beginline.y, endline.y) + 8 &&
        node.y >= Math.min(beginline.y, endline.y - 8)
      ) {
        //判断y坐标是否在线段内
        if (long < 8) {
          //判断点到线距离
